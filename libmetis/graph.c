...
{
    my_graph_t* graph = (my_graph_t*)gk_malloc(sizeof(my_graph_t), "CreateMyGraph: graph");

    graph->n = n;
    graph->m = 0;
    graph->m_max = m_max;
    graph->nx = imalloc(m_max, "CreateMyGraph: nx");
    graph->adj = imalloc(m_max, "CreateMyGraph: adj");
    graph->prev = imalloc(m_max, "CreateMyGraph: prev");
    graph->inv_first = imalloc(m_max, "CreateMyGraph: prev");
    graph->first = imalloc(n, "CreateMyGraph: first");
    graph->counts = imalloc(n, "CreateMyGraph: counts");
...
{
    my_graph_t* graph = wspacemalloc(ctrl, sizeof(my_graph_t));

    graph->n = n;
    graph->m = 0;
    graph->m_max = m_max;
    graph->nx = iwspacemalloc(ctrl, m_max);
    graph->adj = iwspacemalloc(ctrl, m_max);
    graph->prev = iwspacemalloc(ctrl, m_max);
    graph->inv_first = iwspacemalloc(ctrl, m_max);
    graph->first = iwspacemalloc(ctrl, n);
    graph->counts = iwspacemalloc(ctrl, n);
...
{
    gk_free((void**)&graph->nx, &graph->adj, &graph->prev, &graph->first, &graph->inv_first, &graph->counts, LTERM);
