            ...
            real_t *tpwgts, real_t *ubvec)
{
  ...
  switch (optype) {
    ...
    case METIS_OP_OMETIS:
      ...
      // custom options
      ctrl->ltype = GETOPTION(options, METIS_OPTION_LLEVEL, METIS_MMD);
      ctrl->nIparts     = GETOPTION(options, METIS_OPTION_NIPARTS, LARGENIPARTS);
      ctrl->CoarsenTo   = GETOPTION(options, METIS_OPTION_COARSENTO, 100);
      ctrl->llswitch    = GETOPTION(options, METIS_OPTION_LLSWITCH, MMDSWITCH);
