...
\version\verbatim $Id: coarsen.c 17715 2014-10-02 16:08:42Z dominique $ \endverbatim
*/
...
graph_t *CoarsenGraph(ctrl_t *ctrl, graph_t *graph)
{
  ...
  IFSET(ctrl->dbglvl, METIS_DBG_TIME, gk_startwctimer(ctrl->CoarsenTmr));
  IFSET(ctrl->dbglvl, METIS_DBG_TIME, ctrl->CoarsenCount++)
  ...
  IFSET(ctrl->dbglvl, METIS_DBG_COARSEN, PrintCGraphStats(ctrl, graph));
  IFSET(ctrl->dbglvl, METIS_DBG_TIME, gk_stopwctimer(ctrl->CoarsenTmr));
...
idx_t Match_RM(ctrl_t *ctrl, graph_t *graph)
{
  ...
  IFSET(ctrl->dbglvl, METIS_DBG_TIME, gk_startwctimer(ctrl->MatchTmr));
  IFSET(ctrl->dbglvl, METIS_DBG_TIME, ctrl->MatchCount++)
  ...
  /* Determine a "random" traversal order that is biased towards 
     low-degree vertices */
  my_irandArrayPermute_r(nvtxs, tperm, nvtxs/8, 1, &ctrl->curseed);
  ...
  IFSET(ctrl->dbglvl, METIS_DBG_TIME, gk_stopwctimer(ctrl->MatchTmr));
         ...
         idx_t *match)
{
  ...
  IFSET(ctrl->dbglvl, METIS_DBG_TIME, gk_startwctimer(ctrl->ContractTmr));
  IFSET(ctrl->dbglvl, METIS_DBG_TIME, ctrl->ContractCount++)
  ...
  IFSET(ctrl->dbglvl, METIS_DBG_TIME, gk_stopwctimer(ctrl->ContractTmr));
