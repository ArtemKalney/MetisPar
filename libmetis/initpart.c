...
void InitSeparator(ctrl_t *ctrl, graph_t *graph, idx_t niparts) 
{
  ...
  IFSET(ctrl->dbglvl, METIS_DBG_TIME, ctrl->InitPartCount++)
         ...
         idx_t niparts)
{
  ...
  for (inbfs=0; inbfs<niparts; inbfs++) {
    ...
    if (inbfs > 0) {
      my_irandArrayPermute_r(nvtxs, perm, nvtxs/2, 1, &ctrl->curseed);
         ...
         idx_t niparts)
{
  ...
  for (inbfs=0; inbfs<niparts; inbfs++) {
    ...
    queue[0] = my_irandInRange_r(nvtxs, &ctrl->curseed);
    touched[queue[0]] = 1;
    first = 0; 
    last  = 1;
    nleft = nvtxs-1;
    ...
    for (;;) {
      if (first == last) { /* Empty. Disconnected graph! */
        ...
        k = my_irandInRange_r(nleft, &ctrl->curseed);
        for (i=0; i<nvtxs; i++) {
          if (touched[i] == 0) {
            if (k == 0)
              break;
    ...
    if (pwgts[1] == 0) 
      where[my_irandInRange_r(nvtxs, &ctrl->curseed)] = 1;
    if (pwgts[0] == 0) 
      where[my_irandInRange_r(nvtxs, &ctrl->curseed)] = 0;
